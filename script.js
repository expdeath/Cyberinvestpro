...
const Utils = {
    ...
    // Generate unique and diverse colors for charts
    generateThemeColors(count) {
        const colors = [];
        const usedHues = new Set();
        const minHueDistance = Math.max(15, 360 / (count * 1.5)); // Ensure good separation
        
        // Expanded color palette covering full spectrum with vibrant options
        const colorCategories = [
            // Purples & Violets
            { h: 250, s: 75, l: 60 }, { h: 270, s: 70, l: 65 }, { h: 290, s: 80, l: 60 },
            // Blues
            { h: 210, s: 85, l: 55 }, { h: 190, s: 80, l: 60 }, { h: 230, s: 75, l: 50 },
            // Teals & Cyans
            { h: 170, s: 70, l: 55 }, { h: 180, s: 85, l: 50 }, { h: 160, s: 75, l: 60 },
            // Greens
            { h: 120, s: 70, l: 55 }, { h: 140, s: 80, l: 50 }, { h: 100, s: 75, l: 60 },
            // Yellows & Golds
            { h: 50, s: 85, l: 60 }, { h: 40, s: 90, l: 55 }, { h: 60, s: 80, l: 65 },
            // Oranges
            { h: 25, s: 85, l: 60 }, { h: 15, s: 90, l: 65 }, { h: 35, s: 80, l: 55 },
            // Reds & Pinks
            { h: 350, s: 80, l: 60 }, { h: 330, s: 85, l: 65 }, { h: 10, s: 75, l: 55 },
            // Magentas
            { h: 310, s: 80, l: 60 }, { h: 320, s: 75, l: 65 }
        ];
        ...
        for (let i = 0; i < count; i++) {
            ...
            do {
                ...
                if (attempts < 20) {
                    // First try: use expanded palette with variations
                    const baseIndex = Math.floor(Math.random() * colorCategories.length);
                    const base = colorCategories[baseIndex];
                    
                    // Add more variation for uniqueness
                    const hueVariation = (Math.random() - 0.5) * 60; // ±30 degrees
                    const satVariation = (Math.random() - 0.5) * 30; // ±15%
                    const lightVariation = (Math.random() - 0.5) * 25; // ±12.5%
                    
                    h = (base.h + hueVariation + 360) % 360; // Wrap around
                    s = Math.max(50, Math.min(95, base.s + satVariation));
                    l = Math.max(35, Math.min(75, base.l + lightVariation));
                } else {
                    // Fallback: completely random but vibrant colors
                    h = Math.random() * 360;
                    s = 60 + Math.random() * 35; // 60-95%
                    l = 40 + Math.random() * 30; // 40-70%
                }
                
                // Check for minimum distance from existing hues
                let tooClose = false;
                for (const usedHue of usedHues) {
                    const distance = Math.min(
                        Math.abs(h - usedHue),
                        360 - Math.abs(h - usedHue)
                    );
...
class AppInitializer {
    ...
    _setupEventListeners() {
        ...
        // Inventory toggle with smooth animation
        ...
        if (inventoryToggle && inventoryCard) {
            ...
            inventoryToggle.addEventListener('change', Utils.debounce((e) => {
                if (e.target.checked) {
...
function initAiAllocationChart(labels, data) {
    ...
    // Generate dynamic theme-consistent colors for each data item
    const dynamicColors = Utils.generateThemeColors(data.length);
        ...
        data: {
            ...
            datasets: [{
                ...
                hoverBackgroundColor: dynamicColors.map(color => {
                    // Create hover effect by adjusting lightness
                    const hsl = color.match(/hsl\((\d+),\s*(\d+)%,\s*(\d+)%\)/);
                    if (hsl) {
                        const [, h, s, l] = hsl;
                        return `hsl(${h}, ${s}%, ${Math.min(85, parseInt(l) + 10)}%)`;
                    }
                    return color;
                }),
                hoverBorderWidth: 6
